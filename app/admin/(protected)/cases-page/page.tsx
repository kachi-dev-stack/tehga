...
export default function CasesPageAdmin() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await api.saveCasesPage(draft);
      toast.success("Cases page saved");
    } catch {
  ...
  return (
    <>
      <PageHeader
        title="Cases Page"
        description="Hero, intro paragraph and CTA banner on the Work page."
      />
      {loading ? (
        ...
        <div className="rounded-sm border border-dashed border-destructive/30 bg-destructive/5 p-12 text-center">
          <p className="text-sm text-destructive">
            Failed to load cases page content.
          </p>
          <button
            onClick={load}
        ...
        <form onSubmit={save} className="space-y-6">
          ...
          <Card className="space-y-5">
            ...
            <p className="text-xs text-muted-foreground">
              Renders as: "[before] <em>emphasis</em>" (emphasis is italicised)
            </p>
