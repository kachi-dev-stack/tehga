...
export default function IndustriesPageAdmin() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await api.saveIndustriesPage(draft);
      toast.success("Industries page saved");
    } catch {
  ...
  return (
    <>
      <PageHeader
        ...
        description="Hero, intro paragraph and CTA banner on the Industries page."
      />
      {loading ? (
        ...
        <div className="rounded-sm border border-dashed border-destructive/30 bg-destructive/5 p-12 text-center">
          <p className="text-sm text-destructive">
            Failed to load industries page content.
          </p>
          <button
            onClick={load}
